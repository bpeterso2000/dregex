"""A pure Python implementation of the _sre module.

Copyright 2005 Nik Haldimann, licensed under the MIT license

This code is based on material licensed under CNRI's Python 1.6 license.
...
from sre_constants import (
    MAXREPEAT, ATCODES, OPCODES, CHCODES,
    SRE_INFO_PREFIX, SRE_INFO_LITERAL,
    SRE_FLAG_UNICODE, SRE_FLAG_LOCALE
)

from dbug import OpTracker, disp_str_pos, op_logger, show_match

if sys.version_info[:2] < (3, 3):
    print('Requires Python 3.3+.', file=sys.stderr)
    sys.exit(1)

CODESIZE = 4
CODESETS = (ATCODES, OPCODES, CHCODES)

# Not normally a fan of globals, but makes for a clean port from brython
# with added support for sre_constants across different Python versions.
for codes in CODESETS:
    for code in codes:
        globals()[code.name] = code.name.lower()

ATCODES, OPCODES, CHCODES = (
    {i.name.lower(): n for n, i in enumerate(codes)} for codes in CODESETS
)


def compile(pattern, flags, code, groups=0, groupindex={}, indexgroup=[None]):
    """Compiles (rather just converts) pattern descriptor to SRE_Pattern.

    Note: Actual compilation of opcodes happens in sre_compile.
    """
...
def getlower(char_ord, flags):
    if (
        (char_ord < 128) or
        (flags & SRE_FLAG_UNICODE) or
        (flags & SRE_FLAG_LOCALE and char_ord < 256)
    ):
...
class SRE_Pattern:
    ...
    def __init__(
        self, pattern, flags, code, groups=0, groupindex={}, indexgroup=[None]
    ):
    ...
    def search(self, string, pos=0, endpos=sys.maxsize, method='search'):
        ...
        result = SRE_Match(self, state) if match else None
        print(show_match(result))
        return result
    ...
    def _subx(self, template, string, count=0, subn=False):
        ...
        if not callable(template) and "\\" in template:
            # handle non-literal strings;
            # hand it over to the template compiler
            import re as sre
        ...
        while not count or n < count:
            ...
            if not (
                last_pos == state.start and
                last_pos == state.string_position and
                n > 0
            ):
    ...
    def subn(self, repl, string, count=0):
        """Same as sub, but also return number of substitutions made.

        :rtype: tuple (new_string, number_of_subs_made)
        """
...
class SRE_Match:
    ...
    def __init__(self, pattern, state):
        ...
        if (
            self.lastindex is not None and
            pattern._indexgroup and
            0 <= self.lastindex < len(pattern._indexgroup)
        ):
            # The above upper-bound check should not be necessary, as
            # the re compiler is supposed to always provide an
            #_indexgroup list long enough. But the re.Scanner class
            # seems to screw up something there, test_scanner in
            # test_re won't work without upper-bound checking. XXX
            # investigate this and report bug to CPython.
            self.lastgroup = pattern._indexgroup[self.lastindex]
        else:
            self.lastgroup = None

    def _create_regs(self, state):
        """Creates tuple of index pairs representing matched groups."""
        regs = [(state.start, state.string_position)]
        for group in range(self.re.groups):
            mark_index = 2 * group
            if (
                mark_index + 1 < len(state.marks) and
                state.marks[mark_index] is not None and
                state.marks[mark_index + 1] is not None
            ):
                regs.append((
    ...
    def start(self, group=0):
        """Returns indices of the start of substring matched by group.
    ...
    def end(self, group=0):
        """Returns indices of the end of substring matched by group.
    ...
    def expand(self, template):
        ...
        ... and group references on template.
        """
        import sre
    ...
    def groups(self, default=None):
        ...
        The default argument is used for groups that did not participate
        in the match (defaults to None).
        """
        groups = []
    ...
    def groupdict(self, default=None):
        """Return a dictionary containing all the named subgroups.
    ...
    def group(self, *args):
        ...
        Each argument is either a group index or a group name.
        """
        if len(args) == 0:
            args = (0,)
        grouplist = []
...
class _State:
    ...
    def match(self, pattern_codes):
        """Optimization: Check string length.

        pattern_codes[3] contains the minimum length for a string to
        possibly match.
    ...
    def search(self, pattern_codes):
        ...
        if pattern_codes[0] == OPCODES["literal"]:
            # This is used for short prefixes.
            _log('SRE_INFO_PREFIX: Pattern starts with a literal character.')
            character = pattern_codes[1]
            while True:
                while (
                    string_position < self.end and
                    ord(self.string[string_position]) != character
                ):
                    string_position += 1
                ...
                _log(disp_str_pos(self.string, self.string_position))
    ...
    def fast_search(self, pattern_codes):
        """Skips forward in a string as fast as possible.

        Uses the information from an optimization info block.
...
class _MatchContext:
    ...
    def push_new_context(self, pattern_offset):
        """Create new child context of this one and push it on stack.

        Pattern_offset is the offset off the current code position to
        start interpreting from.
        """
        child_context = _MatchContext(
            self.state,
            self.pattern_codes[self.code_position + pattern_offset:]
        )
        # print("_sre.py:517:pushing new context")
        # child_context.has_matched)
...
class _OpcodeDispatcher(_Dispatcher):
    ...
    def match(self, context):
        """Returns True if current context matches, False if it doesn't.

        And None if matching is not finished, ie must be resumed after
    ...
    def dispatch(self, opcode, context):
        """Dispatches a context on a given opcode.
    ...
    def op_literal(self, ctx):
        ...
        # <LITERAL> <code>
        self._log(ctx, "LITERAL", chr(int(ctx.peek_code(1))))
    ...
    def op_in_ignore(self, ctx):
        # match set member (or non_member), disregard current char case
        # <IN_IGNORE> <skip> <set>
    ...
    def op_branch(self, ctx):
        ...
        while current_branch_length:
            ...
            if not (
                ctx.peek_code(1) == OPCODES["literal"] and
    ...
    def op_repeat_one(self, ctx):
        ...
        # this operator only works if the repeated item is exactly
        ...
        ctx.has_matched = False
        # ctx.has_matched = True
        # <== this should be True (so match object gets returned to program)
        yield True
    ...
    def op_max_until(self, ctx):
        ...
        if (
            (count < maxcount or maxcount == MAXREPEAT) and
    ...
    def op_min_until(self, ctx):
        ...
        if count >= maxcount and maxcount != MAXREPEAT:
            ctx.has_matched = False
            # print('_sre.py:line1022, update context.has_matched var')
            yield True
        repeat.count = count
        child_context = repeat.push_new_context(4)
        yield False
    ...
    def general_op_groupref(self, ctx, decorate=lambda x: x):
        ...
        while group_start < group_end:
            if (
                ...
                ctx.has_matched = False
                # print('_sre.py:line1042, update context.has_matched var')
                return True
            group_start += 1
            ctx.skip_char(1)
        ctx.skip_code(2)
    ...
    def unknown(self, ctx):
        ...
        raise RuntimeError(
            "Internal re error. Unknown opcode: %s" % ctx.peek_code()
        )
    ...
    def check_charset(self, ctx, char):
        ...
        # _log('check_charset', result)
        return result

    def count_repetitions(self, ctx, maxcount):
        """Returns the number of repetitions of a single item.single

        Starts from the current string position. The code pointer is
        expected to point to a REPEAT_ONE operation (with the repeated
        4 ahead).
        """
        count = 0
        ...
        # XXX could special case every single character pattern here,
        # as in C. This is a general solution, a bit hackisch, but works
        ...
        while count < real_maxcount:
            # this works because the single character pattern is
            # followed by a success opcode
    ...
    def _log(self, context, opname, *args):
        self._dbug.op_count += 1
        for message in op_logger(self, context, opname, args):
            _log(message)
        self._dbug.last_str_pos = context.string_position
        self._dbug.last_marks = copy(context.state.marks)
...
class _CharsetDispatcher(_Dispatcher):
    ...
    def set_charset(self, ctx):
        ...
        if CODESIZE == 2:
            if (
                char_code < 256 and
                ctx.peek_code(char_code >> 4) & (1 << (char_code & 15))
            ):
                return self.ok
            # skip bitmap
            ctx.skip_code(16)
    ...
    def set_bigcharset(self, ctx):
        raise NotImplementedError(
            "_sre.py: set_bigcharset, array not implemented"
        )
        ...
        if char_code < 65536:
            ...
            a.fromstring(
                 array.array(
                    CODESIZE == 2 and "H" or "I",
                    [ctx.peek_code(block_index // CODESIZE)]
                 ).tostring()
            )
...
# All unicode codepoints reported by Py_UNICODE_ISLINEBREAK; static list
